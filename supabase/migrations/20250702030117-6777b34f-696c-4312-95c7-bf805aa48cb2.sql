-- Tạo hệ thống quản lý KPI hoàn chỉnh theo tiêu chuẩn doanh nghiệp
...
ALTER TABLE public.kpi ADD COLUMN IF NOT EXISTS start_date DATE DEFAULT CURRENT_DATE;
...
CREATE TABLE public.kpi_measurements (
  ...
  measurement_period VARCHAR NOT NULL, -- 'Q1 2024', 'Jan 2024', etc.
  notes TEXT,
...
CREATE TABLE public.kpi_targets (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  kpi_id UUID NOT NULL REFERENCES public.kpis(id) ON DELETE CASCADE,
  target_period VARCHAR NOT NULL, -- 'Q1 2024', 'Jan 2024', etc.
...
CREATE INDEX idx_kpis_employee ON public.kpis(employee_id);
...
-- Insert dữ liệu mẫu cho KPI categories
INSERT INTO public.kpi_categories (name, description, color, created_by) VALUES
('Doanh thu', 'Các chỉ số liên quan đến doanh thu và tăng trưởng kinh doanh', '#10B981', (SELECT id FROM public.employees LIMIT 1)),
('Chất lượng', 'Các chỉ số đánh giá chất lượng sản phẩm và dịch vụ', '#3B82F6', (SELECT id FROM public.employees LIMIT 1)),
('Hiệu suất', 'Các chỉ số đánh giá hiệu suất làm việc và năng suất', '#F59E0B', (SELECT id FROM public.employees LIMIT 1)),
('Khách hàng', 'Các chỉ số liên quan đến sự hài lòng và duy trì khách hàng', '#EF4444', (SELECT id FROM public.employees LIMIT 1)),
('Nhân sự', 'Các chỉ số quản lý và phát triển nhân lực', '#8B5CF6', (SELECT id FROM public.employees LIMIT 1)),
('Chi phí', 'Các chỉ số kiểm soát và tối ưu hóa chi phí', '#6B7280', (SELECT id FROM public.employees LIMIT 1)),
('Sáng tạo', 'Các chỉ số đổi mới và phát triển sản phẩm mới', '#EC4899', (SELECT id FROM public.employees LIMIT 1)),
('Quy trình', 'Các chỉ số tối ưu hóa quy trình và vận hành', '#14B8A6', (SELECT id FROM public.employees LIMIT 1));

-- Insert dữ liệu mẫu cho KPI frameworks
INSERT INTO public.kpi_frameworks (name, description, framework_type, target_level, created_by) VALUES
('Khung KPI Công ty', 'Khung KPI tổng thể cho toàn công ty theo chiến lược phát triển', 'company', 'strategic', (SELECT id FROM public.employees LIMIT 1)),
('Khung KPI Phòng ban', 'Khung KPI cho các phòng ban theo mục tiêu vận hành', 'department', 'operational', (SELECT id FROM public.employees LIMIT 1)),
('Khung KPI Nhóm làm việc', 'Khung KPI cho các nhóm làm việc và dự án', 'team', 'tactical', (SELECT id FROM public.employees LIMIT 1)),
('Khung KPI Cá nhân', 'Khung KPI cho từng cá nhân theo chức vụ và nhiệm vụ', 'individual', 'operational', (SELECT id FROM public.employees LIMIT 1));